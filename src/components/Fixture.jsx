      ...
      <div className="bg-[#1a1a1a] rounded-xl p-3 border border-green-900/20">
        {(p.fechaHora || p.cancha) && (
          <div className="flex justify-between text-[10px] text-gray-500 mb-2">
            <span>{formatFechaHora(p.fechaHora) || 'Horario a confirmar'}</span>
      ...
      <div className="bg-gradient-to-b from-green-900/40 to-[#0a0a0a] px-4 pt-6 pb-4">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {FASES.map(f => (
            <button
              key={f.id}
              onClick={() => { setFase(f.id); if (f.id === 'liga' && fechasLiga.length) setFechaSel(fechasLiga[0]) }}
            ...
            {fechasLiga.map(n => (
              <button
                ...
                className={`w-10 h-10 rounded-full flex-shrink-0 text-sm font-bold transition-all
                  ${fechaSel === n ? 'bg-green-600 text-white' : 'bg-[#1a1a1a] text-gray-400 border border-green-900/30'}`}
              >
                {n}
              </button>
            ))}
